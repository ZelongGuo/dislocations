...
void CalTriStrains(const double sx, const double sy, const double sz, const double *x, const double *y, const double *z, const double pr, const double ss, const double ts, const double ds, double *E) {
	...
	*(E + i) = 0.0;
	...
	if (fabs(beta) > 0.000001 && fabs(beta - M_PI) > 0.000001) {
            ...
            advs(sx1, sy1, sz - z_temp[iTri], z_temp[iTri], beta, pr, lss, lts, lds, &a11, &a22, &a33, &a12, &a13, &a23);  

            /* Second angular dislocation */
            double sx2, sy2;
            RotateXyVec(sx - x_temp[iTri+1], sy - y_temp[iTri + 1], -strike, &sx2, &sy2);
            double b11, b22, b33, b12, b13, b23;
            advs(sx2, sy2, sz - z_temp[iTri + 1], z_temp[iTri + 1], beta, pr, lss, lts, lds, &b11, &b22, &b33, &b12, &b13, &b23); 
