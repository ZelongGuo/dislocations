...
int isPointInTriangle2D(double px, double py, const Vector3 *a,
  ...
  double alpha =
      ((b->y - c->y) * (px - c->x) + (c->x - b->x) * (py - c->y)) / denominator;
  double beta =
      ((c->y - a->y) * (px - c->x) + (a->x - c->x) * (py - c->y)) / denominator;
...
void adv(double y1, double y2, double y3, double a, double beta, double nu,
  ...
  double v2CB3 =
      (1 - 2 * nu) *
          (-sinbeta * log(Rbar + z3bar) - y1 / (Rbar + y3bar) * (1 + a / Rbar) +
...
void CalTriDisps(const double sx, const double sy, const double sz, double *x,
  ...
  for (int iTri = 0; iTri < 3; ++iTri) {
    ...
    double strike =
        180 / M_PI *
        atan2(y_temp[iTri + 1] - y_temp[iTri], x_temp[iTri + 1] - x_temp[iTri]);
    ...
    if (fabs(beta) > 0.000001 && fabs(beta - M_PI) > 0.000001) {
      ...
      double ux1, uy1, uz1;
      adv(sx1, sy1, sz - z_temp[iTri], z_temp[iTri], beta, pr, lss, lts, lds,
          &ux1, &uy1, &uz1);

      /* Second angular dislocation */
      double sx2, sy2;
      RotateXyVec(sx - x_temp[iTri + 1], sy - y_temp[iTri + 1], -strike, &sx2,
                  &sy2);
      double ux2, uy2, uz2;
      adv(sx2, sy2, sz - z_temp[iTri + 1], z_temp[iTri + 1], beta, pr, lss, lts,
          lds, &ux2, &uy2, &uz2);
...
void CalTriStrains(const double sx, const double sy, const double sz, double *x,
   ...
   *
   * Output:
   * E : [1 x 9], STRAIN components, dimensionless, 6 of them are independent
   *
   * */
  ...
  for (int iTri = 0; iTri < 3; ++iTri) {
    ...
    if (fabs(beta) > 0.000001 && fabs(beta - M_PI) > 0.000001) {
      ...
      advs(sx1, sy1, sz - z_temp[iTri], z_temp[iTri], beta, pr, lss, lts, lds,
      ...
      advs(sx2, sy2, sz - z_temp[iTri + 1], z_temp[iTri + 1], beta, pr, lss,
           lts, lds, &b11, &b22, &b33, &b12, &b13, &b23);
