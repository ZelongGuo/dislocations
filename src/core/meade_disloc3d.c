...
void meade_disloc3d(double *models, int nmodel, double *obss, int nobs,
                    double mu, double nu, double *U, double *D, double *S,
                    double *E, int *flags) {
  /*
   * Input Parameters:
   *
   ...
   * str-slip, dip-selip, opening obss  : [nobs * 3], a pointer of 1-D array, in
   * which the Z <= 0 mu    : shear modulus nu    : Poisson's ratio
   *
   * Output:
   * U     : [nobs x 3], DISPLACEMENT, the unit is same to those defined by
   * dislocation slip in models D     : [nobs x 9], 9 spatial derivatives of the
   * displacements having 3 elements S     : [nobs x 9], STRESS, the unit
   * depends on that of shear modulus, 6 of them are independent E     : [nobs x
   * 9], STRAIN, dimensionless, 6 of them are independent flags : [nobs *
   * nmodle], a pointer of an 1-D array 0 normal; 1 the Z value of the obs > 0
   *         10 the depth of the fault upper center point reached to surface
   * (depth < 0) 100 singular point (observation point lies on the fault edges);
  ...
  for (i = 0; i < nobs; i++) {
    ...
    for (j = 0; j < nmodel; j++) {
      ...
      // Apply some translations to transfer Observation Cartesian to Fault
      // Coordinate
      ...
      if ((model[3] <= 0.0) || (model[4] <= 0.0) || (depth < 0.0)) {
        ...
        fprintf(
            stderr,
            "Error, unphysical model! Check fault length, width and the center "
            "point depth on upper fault edge should all be positive values.\n");
      ...
      // printf("X = %f, y = %f, depth = %f, length = %f, width = %f, strkie =
      // %f, dip = %f, ss=%f, ds=%f, opening=%f\n", model[0], model[1],
      // model[2], model[3], model[4], model[5], model[6], model[7], model[8] ,
      // model[9]);

      dc3d_(&alpha, &x, &y, &z, &depth, &dip, &al1, &al2, &aw1, &aw2, &disl1,
            &disl2, &disl3, &ux, &uy, &uz, &uxx, &uyx, &uzx, &uxy, &uyy, &uzy,
            &uxz, &uyz, &uzz, &iret);
      // printf("alpha: %f, x:%f, y:%f, z:%f, depth:%f, dip:%f, al1:%f, al2:%f,
      // aw1:%f, aw2:%f, dis1:%f, dis2:%f, dis3:%f\n", alpha, x, y, z, depth,
      // dip, al1, al2, aw1, aw2, disl1, disl2, disl3);
